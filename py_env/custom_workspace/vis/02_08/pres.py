...
def p(X, labels, colors, linestyles, title, fig, ax, ylim, ylabel):
    ...
    # cellText = [[r"$\approx$ " + str(x) for x in major_ticks_x], ["21s", "53s", "85s", r"$\approx$ 120s", r"$\approx$ 153s"]]
    cellText = [[str(x) for x in major_ticks_x], ["21s", "53s", "85s", "120s", "153s"]]
    colLabels = ["", "", "", "", "", ""]
    the_table = plt.table(
        cellText=cellText,
        rowLabels=["Training trials", "EEGNet train time"],
        # rowColours=[c1, c2],
        colLabels=colLabels,
        loc="bottom",
        cellLoc="center",
    ...
    ax.set_xticklabels([])
    yticklabels = [int(x * 100) / 100 for x in major_ticks_y]
...
def main():
    fig, ax = plt.subplots()
    w, h = plotting.get_dimensions(300, 1, False)
