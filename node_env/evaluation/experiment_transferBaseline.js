...
async function exec(riemann) {
    // ------------------- CONFIG ---------------------
    const benchmarkFileName = "transferBaseline";
    ...
    const configs = ["transfer-baseline"] // "no-transfer", "transfer", "transfer-baseline"
    const transferNBreaksMemories = [25] 
    const sameMetric = true; // will priorize transfer mean metric getAllMeanMetrics(riemann)
    const trainingMeanMetrics = getAllMeanMetrics(riemann); // [riemann.EMetric.ALE, riemann.EMetric.Euclidian, riemann.EMetric.Harmonic, riemann.EMetric.Identity, riemann.EMetric.Kullback, riemann.EMetric.LogDet, riemann.EMetric.LogEuclidian, riemann.EMetric.Riemann, riemann.EMetric.Wasserstein]
    const transferMeanMetrics = undefined;
