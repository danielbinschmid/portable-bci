...
export async function evaluate() {
    ...
    for (const run of runs) {
        ...
        for (const subject of subjects) 
        {
            ...
            const [eegnetX, eegnetY] = prepareNNFinetune(test_data, testsession_labels, 10)
            ...
            // no finetune: 60 % acc
            // TODO: fit model with 10 trials -> 62 percent accuracy
            ...
            const [X_finetune, finetune_labels, X_test, test_labels] = split(X_testsession.unstack(), testsession_labels, 60);

            /**
             * 
             
            const pretraindata = []
            const pretrainlabels = []
            for (const subj of subjects) {
                if (subj != subject) {
                    const d = require("/mnt/d/bachelor-thesis/git/portable-bci/node_env/evaluation/data/eegnet_experiment/hdcaddon/"+subj+"/subj_data.json")
                    
                    const batch = tf.tensor3d(d["train_data"], [d["train_data"].length, nChannels, nSamples]).reshape([d["train_data"].length, nChannels, nSamples, 1]);
                    const X_crossubj = newmodel.predictOnBatch(batch).unstack()
                    pretraindata.push(...X_crossubj)
                    pretrainlabels.push(...d["train_labels"])
                }
                
            }
            await hdc.fit(tf.stack(pretraindata), pretrainlabels, true, .1, 10)
            await hdc.retrain(X_finetune, finetune_labels, 0.1, 6)
            */
            await hdc.fit(X_finetune, finetune_labels, true, .1, 2)

            accs[run][subject]["testAcc"] = getacc(await hdc.predictOnBatch(X_test), test_labels, "testAccOnlyNewData")

            // avgRef += accs[run][subject]["refAcc"] * (1 / subjects.length)
